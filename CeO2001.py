...
fb = open("/Users/mne081069/Desktop/data/chg/100b/CHGCAR",'r',encoding="utf-8")
fs = open("/Users/mne081069/Desktop/data/chg/100s/CHGCAR",'r',encoding="utf-8")
...
img = ax1.imshow(newz, interpolation='spline36', origin='lower', extent=extent,cmap=color)#,vmin=-0.0005,vmax=0.00065
...
for i in range(40,50,10):
    for j in range(0,90,10):
        ...
        ax3d.view_init(i, j)
        ax3d.set_xlim3d(np.min(newx), np.max(newx))
        ax3d.set_ylim3d(np.min(newy), np.max(newy))
        ax3d.set_zlim3d(-0.0019, 0.0015)
        ax3d.plot_surface(newmx, newmy, newz, cmap=color, vmin=-0.0005,vmax=0.00065)#, vmin=-0.0005,vmax=0.00065       
        ax3d.contourf(newmx, newmy, newz, 100 , cmap=color,extent=extent, zdir='z', offset=-0.002,vmin=-0.0005,vmax=0.00065)#,vmin=-0.0005,vmax=0.00065
